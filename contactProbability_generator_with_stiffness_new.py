...
# of input SMC positions assuming Gaussian chain statistics, with additional modifications
# for changes in stiffness between linkers and loops, and adding a Hurst exponent.
...
from looplib.looplib import looptools as lt

class ChainLayout:
    def __init__(self,SMCs,N):
        self.SMCs = sorted(SMCs) 
        #self.SMCs = SMCs[np.argsort(SMCs[:, 0])] 
        self.N = N
        # loops
        ...
        self.children = lt.get_loop_branches(self.parents)
        self.isroot = lt.get_roots(SMCs)
    ...
    def main_circ_chain_dist(self,pos1,pos2):
        ...
        # we determine two path lengths: len1 in the direction 0-N and len2 in the opposite direction
        ...
        if mc1_idx == mc2_idx:
            ...
            len1 = L_right + L_left + L_between
            
          
        L_right = self.main_chain[mc2_idx][1]-pos2
        L_left =  pos1-self.main_chain[mc1_idx][0]
        if mc1_idx>0:
            L_between = self.cum_MCL[-1]-self.cum_MCL[mc2_idx]+self.cum_MCL[mc1_idx-1]
        else:
            L_between = self.cum_MCL[-1]-self.cum_MCL[mc2_idx]

        len2 = L_right + L_left + L_between
        
        
        return len1*len2/(len1+len2)

    
    def main_chain_dist(self,pos1,pos2,circular=False):
        
        if circular==True:
            return self.main_circ_chain_dist(pos1,pos2)
        else:
    ...
    def get_dist(self,pos1,pos2,dist=0,H=1/2,lp=1,circular=False):
        """
        Parameters
        ----------
        pos1 : int
            A monomer position on the polymer chain. 0 <= `pos1` < N.
           
        pos2 : int
            A monomer position on the polymer chain. 0 <= `pos2` < N.
            
        H : float
            The Hurst exponent for the polymer chain. For Gaussian chains 
            (default value) H = 1/2, for "fractal globule" chains, H = 1/3.
            
        lp : float
            The stiffness of "linkers" between loops. The default stiffness
            is 1 (i.e. no difference in stiffness). Stiff linkers have lp > 1,
            and floppy linkers (i.e. stiff loops) have 0 < lp < 1. 
            
        circular: bool
            If True, the distance along the main chain is computed as the shortest
            one on a ring
            
        Output
        ------
        dist : float
            Effective genomic distance between the monomer chains. The contact 
            probability can be obtained by exponentiating `dist` by -3/2. 
        ...
        # re-order pos1 and pos2 so that pos1 < pos2
        if pos1 > pos2:
            pos1, pos2 = (pos2,pos1)
        ...
        if branch1_idx == branch2_idx == -2:
            dist += (self.main_chain_dist(pos1,pos2,circular=circular)*lp)**h
        ...
        # if left position is on the main chain
            ...
            dist_lin = self.main_chain_dist(pos1,root_pos,circular=circular)
            # return sum
            dist += dist_circ**h + (dist_lin*lp)**h
        ...
        # if right position is on the main chain
            ...
            # compute distance of branch1 point to root
            dist_lin = self.main_chain_dist(pos2,root_pos,circular=circular)
            # return sum
            dist += dist_circ**h + (dist_lin*lp)**h
        ...
        # if both positions are on a loop
            ...
            if len(overlap)>0:
                ...
                # if one loop is nested within the other
                else:
                ...
                d1, np1 = self.get_circle_distance(pos1,lp_idx=branch1_idx)
                d2, np2 = self.get_circle_distance(pos2,lp_idx=branch2_idx)
                d3 = self.main_chain_dist(np1,np2,circular=circular)
                
                dist = d1**h + d2**h + (d3*lp)**h 

        return dist

    def get_dist_hs(self,pos1,pos2,dist=0,H=1/2,lp=1,Hs=1/2):
        """
        Parameters
        ----------
        pos1 : int
        ...
        branch2_idx, branch2_all = self.get_loop_idx(pos2)
         ...
         # if both positions are on the main chain
        if branch1_idx == branch2_idx == -2:
            dist += (self.main_chain_dist(pos1,pos2)*lp)**hs
        ...
        # if left position is on the main chain
            ...
            # return sum
            dist += dist_circ**h + (dist_lin*lp)**hs
